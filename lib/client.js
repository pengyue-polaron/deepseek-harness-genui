		...
		children: [
			...
			open ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: t("design.hint") }),
...
const zh = {
	...
	"progress.create.check": "正在检查交互与布局",
	"progress.update.prepare": "正在准备这次修改",
	"progress.update.build": "正在构建候选版本",
	"progress.update.check": "正在确认修改可以安全使用",
...
const en = {
	...
	"progress.create.check": "Checking interaction and layout",
	"progress.update.prepare": "Preparing this change",
	"progress.update.build": "Building a candidate version",
	"progress.update.check": "Checking that the change is safe to use",
...
function isGenuiReadyMessage(event, frameWindow, artifactId, versionId) {
	...
	return value.source === "dsh-genui" && value.type === "ready" && value.artifactId === artifactId && value.versionId === versionId;
}
...
function GenuiToolView({ block, callId, sessionId, t }) {
	...
	(0, react.useEffect)(() => {
		...
		const timeout = window.setTimeout(() => setFrameState((state) => state === "loading" ? "failed" : state), 12e3);
