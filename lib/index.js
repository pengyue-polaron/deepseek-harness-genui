...
//#endregion
//#region src/artifacts/browser-verifier.ts
async function verifyWithBrowser(browser, url) {
	let context;
	const diagnostics$1 = [];
	const notes = [];
	try {
		...
		page.on("console", (message) => {
			if (message.type() === "error") diagnostics$1.push({
				severity: "error",
				text: `browser console: ${message.text()}`
			});
			if (message.type() === "warning") diagnostics$1.push({
				severity: "warning",
				text: `browser console: ${message.text()}`
			});
		});
		page.on("pageerror", (error) => diagnostics$1.push({
			severity: "error",
			text: `browser runtime: ${error.message}`
		}));
		...
		try {
			...
			const surfaceSnapshot = () => frame.locator("html").evaluate((element) => {
				...
				return JSON.stringify({
					html: element.querySelector("#root")?.innerHTML ?? "",
					controls
				});
			});
			const inspectSurface = async (label, accessibility = false) => {
				const report = await frame.locator("html").evaluate((element, includeAccessibility) => {
					...
					const labelledBy = (candidate) => (candidate.getAttribute("aria-labelledby") ?? "").split(/\s+/).filter(Boolean).map((id) => document.getElementById(id)?.textContent?.trim() ?? "").join(" ").trim();
					const nameOf = (candidate) => {
						...
						if (candidate instanceof HTMLInputElement || candidate instanceof HTMLSelectElement || candidate instanceof HTMLTextAreaElement) {
							const labels = [...candidate.labels ?? []].map((label$1) => label$1.textContent?.trim() ?? "").join(" ").trim();
							...
							if (candidate instanceof HTMLInputElement && [
								...
								"reset",
								"image"
							].includes(candidate.type)) return candidate.value.trim() || candidate.getAttribute("alt")?.trim() || "";
							return "";
						}
					...
					if (includeAccessibility) {
						for (const [index, candidate] of [...document.querySelectorAll("button, a[href], input:not([type=\"hidden\"]), select, textarea, summary, [role=\"button\"], [role=\"link\"], [role=\"checkbox\"], [role=\"radio\"], [role=\"switch\"], [tabindex]:not([tabindex=\"-1\"])")].entries()) if (visible(candidate) && nameOf(candidate).length === 0) {
						...
						for (const [index, image] of [...document.querySelectorAll("img")].entries()) if (visible(image) && !image.hasAttribute("alt")) missingAlt.push(`img #${index + 1}`);
					}
					return {
				...
				if (report.scrollWidth > report.width) diagnostics$1.push({
					severity: "error",
					text: `${label} viewport overflows horizontally: ${report.scrollWidth}px > ${report.width}px`
				});
				else notes.push(`${label} mounted at ${report.width}px without horizontal overflow`);
				for (const control of report.unlabeled) diagnostics$1.push({
					severity: "error",
					text: `visible interactive control has no accessible name: ${control}`
				});
				for (const image of report.missingAlt) diagnostics$1.push({
					severity: "error",
					text: `visible image is missing alt text: ${image}`
				});
			};
			...
			if (primary.controls > 0) if (primary.marked !== 1) diagnostics$1.push({
				severity: "error",
				text: `interactive app must have exactly one visible data-genui-primary-action control; found ${primary.marked}`
			});
			else if (!primary.markedInteractive) diagnostics$1.push({
				severity: "error",
				text: "data-genui-primary-action must be placed on an interactive control"
			});
			else {
				const before = await surfaceSnapshot();
				let sdkActionObserved = false;
				const observeAction = (request$1) => {
					if (/\/(?:tool|external|state\/write)$/.test(new URL(request$1.url()).pathname)) sdkActionObserved = true;
				};
				...
				if (control.tag === "select") {
					const next = (await action.locator("option").evaluateAll((options) => options.map((option) => option.value)))[1];
					if (next !== void 0) await action.selectOption(next);
				} else if (control.tag === "textarea" || control.tag === "input" && [
					"text",
					"search",
					"email",
					"url",
					"tel"
				].includes(control.type)) await action.fill("Verification input");
				else if (control.tag === "input" && ["range", "number"].includes(control.type)) await action.evaluate((element) => {
					const input = element;
				...
				else diagnostics$1.push({
					severity: "error",
					text: "data-genui-primary-action did not change visible app state or invoke a verified action"
				});
			}
			...
			diagnostics$1.push({
				severity: "error",
				text: `sandboxed preview did not mount: ${error instanceof Error ? error.message : String(error)}`
			});
		}
	} catch (error) {
		diagnostics$1.push({
			severity: "error",
			text: `browser verification failed: ${error instanceof Error ? error.message : String(error)}`
		});
	} finally {
		await context?.close();
	}
	return {
		ok: diagnostics$1.every((item) => item.severity !== "error"),
		diagnostics: diagnostics$1,
		notes
	};
}
var BrowserVerifier = class {
	browser;
	constructor(launchBrowser = () => chromium.launch({ headless: true })) {
	...
	async verify(url) {
		try {
			...
			if (!browser.isConnected()) this.browser = void 0;
			return result;
		} catch (error) {
			this.browser = void 0;
			return {
	...
	async getBrowser() {
		...
		try {
			...
			if (this.browser === current) this.browser = void 0;
			throw error;
		}
	}
};
async function verifyArtifactInBrowser(url) {
...
var DesignStore = class {
	...
	async list() {
		const entries = await readdir(this.root, { withFileTypes: true });
		return (await Promise.all(entries.filter((entry) => entry.isFile() && entry.name.endsWith(".md")).map(async (entry) => {
...
const ARTIFACT_RUNTIME_VERSION = "0.12.2";
...
async function compile(registry, capabilities, routePrefix, previewOrigin, version, agent, delivery, language, verifyBrowser) {
...
function registerGenuiTools(ctx, registry, designs, capabilities, routePrefix, previewOrigin, verifyBrowser = verifyArtifactInBrowser) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const receipt = await compile(registry, capabilities, routePrefix, previewOrigin, await registry.create({
				...
				files: args.files
			}), agent, args.delivery, args.language, verifyBrowser);
...
2. ${defaultDesignId === void 0 ? "For a new app, silently choose and export the best bundled design: editorial-workbench for content, planning, and reading; ledger-grid for comparisons and scheduling; field-atlas for causal, spatial, and scientific explanation; kinetic-signal for live data and connected actions." : `The Harness default design is ${defaultDesignId}. Silently export and use it for new apps unless the user asks for another direction.`}
...
async function apply(ctx, config) {
	...
	return () => {
		capabilities.clear();
		browserVerifier.close().catch(() => void 0);
	};
}
...
//#endregion
export { ArtifactRegistry, Config, DESIGN_PRESETS, DesignStore, apply, buildArtifact, inject, name, verifyArtifactInBrowser };