...
function registerDesignSettingsNamespace(ctx) {
	return ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.settings.register(DESIGN_SETTINGS_NAMESPACE, DesignSettingsMarker);
	});
}
			...
			if (req.method === "POST" && relative[0] === "api" && relative.length >= 3) {
				...
				if (action === "tool") {
					...
					const result = await ctx.tools.execute({
						callId: `genui-${Date.now()}-${Math.random().toString(36).slice(2)}`,
						name: input.name,
...
- Recharts is version 3. Use Tooltip for active chart selection; do not use removed activeIndex props or rely on Customized receiving chart state.
