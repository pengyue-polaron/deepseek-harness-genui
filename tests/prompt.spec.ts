...
describe('GenUI system prompt', () => {
  it('keeps user handoffs product-focused without adding another prompt section', () => {
    ...
    expect(GENUI_SYSTEM_PROMPT).toContain('write one neutral 2–5 word handoff in the user\'s language')
    expect(GENUI_SYSTEM_PROMPT).toContain('write the framing prose before the call and put only that part in the card')
    expect(GENUI_SYSTEM_PROMPT).toContain('run no more tools and end the turn immediately without adding text')
    ...
    expect(GENUI_SYSTEM_PROMPT).toContain('resume on demand by silently calling genui_state_read')
  })
})
