...
- Store user answers, selections, drafts, and progress under concise semantic useArtifactState keys. Keep fetched search results, catalogs, and other replaceable response payloads in plain React useState; persist the user's query or selection, not the full external response. Derive reload and empty-state copy from data that actually remains available. When a later user turn refers to what they entered or selected, resume on demand by silently calling genui_state_read before answering or updating the app; do not create background work to wait for that turn.
