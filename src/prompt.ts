...
- Its runtime contract is: useArtifactState(key, initialValue) returns [value, setValue, { ready, error }]; reportResult(value) saves the concise completed outcome for the next Agent turn; callTool(name, arguments) returns Promise<unknown>; watchTool(name, arguments, onValue, { intervalMs, onError }) returns an unsubscribe function; requestExternal(url, options) returns Promise<{ status, headers, body }>; artifactContext() returns { artifactId, versionId }.
- Store user answers, selections, drafts, and progress under concise semantic useArtifactState keys. Plain React useState is only for disposable presentation state. When a later user turn refers to what they entered or selected, resume on demand by silently calling genui_state_read before answering or updating the app; do not create background work to wait for that turn.
