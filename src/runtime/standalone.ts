...
if (!token) {
  ...
  const copy = language === 'zh'
    ? { read: '读取信息', write: '执行更改', connect: '连接到', methods: '允许请求', queued: '确认后还有 {count} 项访问请求。', failed: '暂时无法完成授权，请重试。' }
  ...
  dialog.addEventListener('cancel', (event) => {
    ...
    if (!allow.disabled) answer(false)
  })
